...
func (s *Server) Start() error {
	...
	if !s.IsLeader {
		go func() {
			...
			fmt.Println("connected to leader at :", s.LeaderAddr)
			if err != nil {
				log.Fatal(err)
			}
			s.readLoop(conn)
...
func (s *Server) handleCommand(conn net.Conn, rawCmd []byte) {
	...
	if err != nil {
		fmt.Println("faield to parse command", err)
...
func (s *Server) sendToFollowers(ctx context.Context, msg *Message) error {
	for conn := range s.followers {
		fmt.Println("forwarding to followers")
		rawMsg := msg.ToBytes()
		fmt.Println("forwarding command ", string(rawMsg))
